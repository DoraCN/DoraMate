...
public static class DoraTelemetry
{
    ...
    /// <summary>
    /// Gets the activity source used by Dora operator spans.
    /// </summary>
    public static readonly ActivitySource ActivitySource = new("DoraMate.DoraOperator", typeof(DoraTelemetry).Assembly.GetName().Version?.ToString());
    ...
    {
        var activityName = string.IsNullOrWhiteSpace(name) ? "dora.operator.process" : name;
    ...
    internal static void ApplyInputTags(Activity? activity, Input input)
    {
        if (activity is null)
        {
            return;
        ...
        activity.SetTag("dora.event.type", "Input");
        activity.SetTag("dora.input.id", input.Id);
        if (input.HasBytes)
        {
            activity.SetTag("dora.payload.kind", "bytes");
        }
        else if (input.HasArrow)
        {
            activity.SetTag("dora.payload.kind", "arrow");
        }
    }
